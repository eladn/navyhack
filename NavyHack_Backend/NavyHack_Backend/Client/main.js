...
{
	$.getJSON('http://192.168.1.165:3000/api/points/'+mmsi, function(response) {
...
function fetchData(bounds)
{
	...
	$.ajax({
	    ...
	    url: 'http://192.168.1.165:3000/api/points/last',
	    type: 'POST',
	    dataType: "json",
	    ...
	    success: function(response) { 
	    	...
	    	// Save to allShips cache array, so next time it won't draw them
	    	for (var i = 0; i < response.length; i++) {
	    		
	    		var current = response[i];

	    		// Add to all ships
	    		if(!allShips[current.mmsi]) {
	    			allShips[current.mmsi] = current;
	    			current.infoView = false;
	    		}
	    	};
	    },
	    error: function() { 
